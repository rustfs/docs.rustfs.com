 ...
 * If you bind-mount host directories with `-v`, every mounted path (data, logs, and TLS certificates) must be owned by `10001:10001`:

```bash
sudo mkdir -p /mnt/rustfs/data /mnt/rustfs/logs
sudo chown -R 10001:10001 /mnt/rustfs/data /mnt/rustfs/logs
```
...
4. If [using TLS](../../integration/tls-configured.md), additional certificate path mounting is needed. The certificate directory must also be readable by UID `10001`:

   ```bash
   sudo chown -R 10001:10001 /path/to/certs
   ```

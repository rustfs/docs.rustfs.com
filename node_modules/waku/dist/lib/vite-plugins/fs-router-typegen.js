...
const parseModule = async (filePath)=>{
    ...
    const lang = filePath.endsWith('.tsx') ? 'tsx' : filePath.endsWith('.ts') ? 'ts' : 'jsx';
    const transformed = await transformWithOxc(source, filePath, {
        lang,
        jsx: 'preserve'
    });
    return (await parse(filePath, transformed.code, {
        lang
    })).program;
};
...
export async function generateFsRouterTypes(pagesDir) {
    ...
    const generateFile = async (filePaths)=>{
        ...
        const lines = [
            ...
            '// prettier-ignore',
            hasAnyGetConfig ? "import type { PathsForPages, GetConfigResponse, SearchCodecsForPages } from 'waku/router';" : "import type { PathsForPages } from 'waku/router';"
        ];
        if (hasAnyGetConfig) {
            lines.push('');
        ...
        lines.push('', '// prettier-ignore', "declare module 'waku/router' {", '  interface RouteConfig {', '    paths: PathsForPages<Page>;', '  }', '  interface CreatePagesConfig {', '    pages: Page;', ...uniqueLayoutPaths.length ? [
            '    layouts: Layout;'
        ] : [], '  }', ...hasAnyGetConfig ? [

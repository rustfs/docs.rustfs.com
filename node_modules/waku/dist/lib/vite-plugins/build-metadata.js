...
export function buildMetadataPlugin({ distDir }) {
    ...
    return {
        ...
        renderChunk (code, chunk) {
            if (code.includes(virtualModule)) {
                assert.equal(this.environment.name, 'rsc');
                const replacement = forceRelativePath(normalizePath(path.relative(path.join(chunk.fileName, '..'), BUILD_METADATA_FILE)));
        ...
        buildApp: {
            async handler (builder) {
                ...
                const buildMetadataFile = joinPath(rootDir, distDir, DIST_SERVER, BUILD_METADATA_FILE);

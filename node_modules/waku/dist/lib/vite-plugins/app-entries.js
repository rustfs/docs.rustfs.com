...
import { getManagedClientEntry, getManagedServerEntry } from '../utils/managed.js';
export function appEntriesPlugin({ srcDir }) {
    return {
        ...
        async resolveId (source, _importer, options) {
            ...
            if (source === 'virtual:vite-rsc-waku/server-entry-inner') {
                const resolved = await this.resolve(`/${srcDir}/${SRC_SERVER_ENTRY}`, undefined, options);
                return resolved ? resolved : '\0' + source;
            }
            if (source === 'virtual:vite-rsc-waku/client-entry') {
                const resolved = await this.resolve(`/${srcDir}/${SRC_CLIENT_ENTRY}`, undefined, options);
                return resolved ? resolved : '\0' + source;
            }
        },
        load (id) {
...
//# sourceMappingURL=app-entries.js.map
...
const hasPluginName = (plugin, name)=>{
    if (!plugin) {
        return false;
    }
...
const excludeOverriddenPlugins = (config, plugins)=>plugins.filter((plugin)=>{
        if (!plugin || Array.isArray(plugin) || !('name' in plugin)) {
            return true;
        }
        return !hasPluginName(config.vite?.plugins, plugin.name);
    });
export function combinedPlugins(config) {
    const ourPlugins = [
        allowServerPlugin(),
        rsc({
            serverHandler: false,
...
//# sourceMappingURL=combined-plugins.js.map
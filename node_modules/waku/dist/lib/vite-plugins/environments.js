...
import { DIST_PUBLIC, DIST_SERVER, SRC_CLIENT_ENTRY, SRC_PAGES, SRC_SERVER_ENTRY } from '../constants.js';
const PKG_NAME = 'waku';
const __dirname = fileURLToPath(new URL('.', import.meta.url));
export function environmentsPlugin(config) {
    return {
        ...
        async config (_config) {
            let viteRscConfig = {
                ...
                define: {
                    ...
                    'import.meta.env.WAKU_CONFIG_RSC_BASE': JSON.stringify(config.rscBase),
                    // CLI has loaded dotenv already at this point
                    ...Object.fromEntries(Object.entries(process.env).flatMap(([k, v])=>k.startsWith('WAKU_PUBLIC_') ? [
                            [
                                `import.meta.env.${k}`,
                                JSON.stringify(v)
                            ],
                ...
                environments: {
                    client: {
                        build: {
                            rolldownOptions: {
                                input: {
                                    index: path.join(__dirname, '../vite-entries/entry.browser.js')
                                }
                            }
                        },
                        optimizeDeps: {
        ...
        configEnvironment (name, environmentConfig, env) {
            ...
            if (environmentConfig.optimizeDeps?.include) {
                environmentConfig.optimizeDeps.include = environmentConfig.optimizeDeps.include.map((name)=>{
            ...
            return {
                resolve: {
                    noExternal: env.command === 'build' ? true : [
                        PKG_NAME
                    ]
                },
                optimizeDeps: {
        ...
        async configureServer (server) {
            ...
            const entryId = environment.config.build.rolldownOptions.input.index;
            return ()=>{
                server.middlewares.use(async (req, res, next)=>{
                    try {
                        ...
                        await getRequestListener((req, ...args)=>mod.INTERNAL_runFetch(process.env, req, ...args))(req, res);
                    } catch (e) {
                        next(e);
                    }
                });

...
const toProcessRequest = (handleRequest)=>async (req)=>{
        ...
        const debugId = import.meta.env.DEV && req.headers.get(DEBUG_ID_HEADER.toLowerCase()) || undefined;
        ...
        const renderUtils = createRenderUtils(temporaryReferences, renderToReadableStream, createFromReadableStream, loadSsrEntryModule, import.meta.env.WAKU_BUILD_ID ?? '', debugChannel, debugId);
        let res;
        try {
...
const toProcessBuild = (handleBuild)=>async ({ emitFile, unstable_registerPrunableFile })=>{
        const renderUtils = createRenderUtils(undefined, renderToReadableStream, createFromReadableStream, loadSsrEntryModule, import.meta.env.WAKU_BUILD_ID ?? '');
        let fallbackHtml;

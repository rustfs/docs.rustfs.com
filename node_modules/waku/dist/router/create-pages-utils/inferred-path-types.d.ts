...
]> : PathPart extends `${infer Prefix}[${string}]${infer Suffix}` ? ReplaceHelper<Rest, StaticSlugs, [
    ...SlugCountArr,
    null
], [
    ...Result,
...
type _GetSlugs<Route extends string, SplitRoute extends string[] = Split<Route, '/'>, Result extends string[] = []> = SplitRoute extends [] ? Result : SplitRoute extends [`${infer MaybeSlug}`, ...infer Rest extends string[]] ? MaybeSlug extends `${string}[${infer Slug}]${string}` ? _GetSlugs<Route, Rest, [...Result, Slug]> : _GetSlugs<Route, Rest, Result> : Result;
...
export type RouteSearch<Path extends string> = Path extends keyof SearchCodecsConfig ? SearchCodecsConfig[Path] extends Unstable_SearchCodec<infer Search> ? Search : never : never;
/**
...
export type SearchCodecsForPages<Pages> = {
    [Page in Extract<Pages, {
        unstable_searchCodec: unknown;
    }> as Page extends {
        path: infer Path extends string;
    } ? Path : never]: Page extends {
        unstable_searchCodec: infer Codec;
    } ? Codec : never;
};
...
export type PropsForPages<Path extends string> = Prettify<Omit<RouteProps<ReplaceAll<Path, `[${string}]`, string>>, 'hash'> & SlugTypes<Path> & SearchProps<Path>>;

...
export function fsRouter(/**
   ...
   */ modules, options) {
    const { pagesDir = 'pages', apiDir = '_api', slicesDir = '_slices', interceptorsDir = '_interceptors', unstable_skipBuild } = options || {};
    return createPages(async ({ createPage, createLayout, createRoot, createApi, createSlice, createInterceptor })=>{
        ...
        for (const file of Object.keys(modules).sort()){
            ...
            if (pathItems.at(0) === interceptorsDir) {
                const interceptorMod = await modules[file]();

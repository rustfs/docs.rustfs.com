---
title: RustFS 다중 서버 다중 디스크 설치
description: 다중 서버의 다중 디스크에 RustFS 설치, 데이터가 다중 서버의 다중 디스크에 저장됩니다.
---

# 다중 서버 다중 디스크 (MNMD, Multiple Node Multiple Disk)

## 1. 설치 전 필독

본 문서에는 RustFS 다중 서버 다중 디스크 모드 배포가 포함되어 있습니다. 다중 서버 다중 디스크는 엔터프라이즈급에서 사용 가능한 성능, 보안성, 확장성에 적합합니다. 그리고 프로덕션 워크로드에 필요한 아키텍처 도표를 제공합니다. 설치 전에는 우리의 시작 모드와 검사 체크리스트를 읽어주시기 바랍니다. 다음과 같습니다:

1. 세 가지 설치 활성화 모드를 명확히 해주십시오:
   - [단일 서버 단일 디스크 모드 (SNSD)](./single-node-single-disk.md)
   - [단일 서버 다중 디스크 모드 (SNMD)](./single-node-multiple-disk.md)
   - [다중 서버 다중 디스크 모드 (MNMD)](./multiple-node-multiple-disk.md) (현재 문서)

2. [설치 전 검사](../checklists/index.md), 각 지표가 프로덕션 지침 특성에 부합하는지 확인하십시오. 프로덕션 표준이 필요하지 않다면 이 지침을 읽지 않아도 됩니다;

최소한 **4대의 서버**가 필요하며, 최소한 각 서버마다 1개의 디스크가 있어야 안전하게 분산 객체 스토리지 클러스터를 시작할 수 있습니다.

다음 아키텍처 도표 예시에서, 데이터는 로드 밸런서를 통해 임의의 서버에 무작위로 기록됩니다. 기본 12 + 4 모드에서, 하나의 데이터 블록은 기본적으로 12개의 데이터 블록 + 4개의 검증 블록으로 분할되어 각기 다른 서버의 다른 디스크에 저장됩니다.

임의의 1대 서버가 손상되거나 유지보수를 받아도 데이터 안전에 영향을 주지 않습니다.

임의의 4개 디스크 이내의 데이터 손상도 데이터 안전에 영향을 주지 않습니다.
...
### 2.3 호스트명 (단일 서버 단일 디스크와 단일 서버 다중 디스크는 이 단계를 건너뛸 수 있습니다)

> 단일 서버 단일 디스크, 단일 서버 다중 디스크는 호스트 별명을 설정하지 않으므로 이 단계를 건너뛰십시오.

RustFS 클러스터를 생성하려면 **동일하고 연속성 있는** 호스트명을 사용해야 합니다. 연속성 있는 호스트명을 구현하는 두 가지 방법이 있습니다:

**1. DNS 구성:**

DNS 해석 서버를 구성하여 이름의 연속성을 보장하십시오.

**2. HOSTS 구성:**

/etc/hosts의 로컬 별명 설정을 수정하십시오. 구체적인 작업은 다음과 같습니다:
